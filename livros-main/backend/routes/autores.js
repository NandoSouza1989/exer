...
router.post("/",async (req,res)=>{
    ...
    if(!nome || !sobrenome || !idade || !data_nascimento || !sexo || !telefone){
        res.status(400).json({msg:"Enviar nome, sobrenome, idade, data_nascimento, e sexo do livro."});
        return;
    }
    ...
    try{
        ...
        const novo = await dbKnex("autores").insert({ome, sobrenome, idade, data_nascimento, sexo,telefone});
